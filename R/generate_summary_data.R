...
#' @param pleiotropy.var A number in \eqn{[0, \infty)} indicating the variance attributed to pleiotropy.
#' @param pleiotropy.cor A number indicating the correlation between correlated horizontal pleiotropy and the specified latent variable. If set to 0, uncorrelated horizontal pleiotropy is generated.
#' @param pleiotropy.cor.exposure A vector of indices of variables that are correlated with the correlated horizontal pleiotropy.
#' @param pleiotropy.cor.effect A vector of effects corresponding to the variables correlated with the correlated horizontal pleiotropy. Specifically, \code{cor(bX[,pleiotropy.cor.exposure] \%*\% pleiotropy.cor.effect, CHP)} equals \code{pleiotropy.cor}.
...
summary_generation=function(theta,m,Rbb,Ruv,Rnn,Nxy,Hxy,LD="identify",non.zero.frac,pleiotropy.frac=0,pleiotropy.var=0.5,pleiotropy.cor=0,pleiotropy.cor.exposure=c(1:length(theta)),pleiotropy.cor.effect=rep(1:length(theta)),effect.dis="normal",pleiotropy.dis="uniform"){
...
indpleio=sample(1:m,pleiotropy.frac*m,replace=F)
eta=as.vector(b0[,pleiotropy.cor.exposure]%*%pleiotropy.cor.effect)
bu=b0[,1]*0
if(pleiotropy.dis=="normal"){
bu1=rnorm(length(indpleio),0,1)
bu1=bu1/sd(bu1)
sd.eta=sd(eta[indpleio])
bu[indpleio]=bu1*sqrt(1-pleiotropy.cor^2)+eta[indpleio]/sd.eta*pleiotropy.cor
}
if(pleiotropy.dis=="uniform"){
bu1=runif(length(indpleio),0,1)
bu1=bu1/sd(bu1)
...
A=list(bX=bX,bXse=bXse,by=by,byse=byse,pleiotropy=pleiotropy0,bX0=b0,by0=c(b0%*%theta+pleiotropy0))
return(A)
}
